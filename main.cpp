...
string convertBase(const string& number, int fromBase, int toBase) {
    if (fromBase < 2 || fromBase > 16 || toBase < 2 || toBase > 16) {
        return "ќшибка: система счислени€ должна быть от 2 до 16";
    }

    if (!isValidNumber(number, fromBase)) {
        return "ќшибка: некорректное число дл€ данной системы счислени€";
    }
...
int main() {
    ...
    cout << "¬ведите исходную систему счислени€ (2-16): ";
    cin >> fromBase;

    cout << "¬ведите целевую систему счислени€ (2-16): ";
    cin >> toBase;
